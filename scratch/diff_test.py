...
if K >= K_min:
    ...
    if K % 2 == 1: Q_suite = sorted_Q[K // 2]
    else: Q_suite = Fraction(sorted_Q[K // 2 - 1] + sorted_Q[K // 2], 2)
...
if K < K_min:
    ...
    if Q_suite >= Fraction(3, 4) and H_0_75 >= H_required: classification = "SUPPORTED"
    elif Q_suite > Fraction(1, 2): classification = "WEAK_SUPPORT"
    else: classification = "FALSIFIED"
...
import json
with open("docs/research/t3/t3b9_matched_analysis.json") as f:
    analysis = json.load(f)

for k in analysis:
    if analysis[k] != expected_artifact[k]:
        print(f"{k} differs")
        print(f"Analysis: {analysis[k]}")
        print(f"Expected: {expected_artifact[k]}")
