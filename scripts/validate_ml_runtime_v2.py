...
def main():
    ...
    print("Running MPS determinism validation V2...")
    ...
    for seed in sorted(FROZEN_SEEDS):
        ...
        for i in range(3):
            proc = subprocess.run(
                ["scripts/run_ml_runtime_v2.sh", __file__, "--run-subprocess", str(seed), "64", "0"],
    ...
    for bs in [1, 2, 17, 63]:
        ...
        for i in range(3):
            proc = subprocess.run(
                ["scripts/run_ml_runtime_v2.sh", __file__, "--run-subprocess", "1729", str(bs), "0"],
                capture_output=True, text=True
            )
            if proc.returncode != 0:
                sys.exit(1)
    ...
    for i in range(3):
        proc = subprocess.run(
            ["scripts/run_ml_runtime_v2.sh", __file__, "--run-subprocess", "1729", "8", "1"],
            capture_output=True, text=True
        )
        if proc.returncode != 0:
            sys.exit(1)
    ...
    with open("artifacts/research/ml_runtime_validation_hashes_v2.json", "w") as f:
