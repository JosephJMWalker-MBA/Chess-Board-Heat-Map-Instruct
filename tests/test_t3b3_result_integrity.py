...
AGG_SHA = "6cf4b2e247b277cbf2d185ebddbb7b6bd3532f95aaf1a7a42ca072b3d287c1c8"
...
def test_t3b3_result_integrity():
    # Manifest integrity
    ...
    with open("tests/fixtures/t3b3/t3b3_corpus_result.json", "rb") as f:
        agg_bytes = f.read()
    assert hashlib.sha256(agg_bytes).hexdigest() == AGG_SHA

    raw = json.loads(raw_bytes.decode("utf-8"))
    agg = json.loads(agg_bytes.decode("utf-8"))
    
    assert raw["actual_search_count"] == 362
    ...
    assert agg["suite_digest"] == suite_digest
    ...
    for i in range(12):
        ...
        with open(result_file_path, "r", encoding="utf-8") as f:
            res_content = f.read()
        res_hash = hashlib.sha256(res_content.encode("utf-8")).hexdigest()
        assert res_hash in agg["result_digests"]
        
        res_json = json.loads(res_content)
        # Parse ExperimentResult
        er = ExperimentResult.model_validate(res_json)
        # Verification built into class:
        ...
        spec_json_str = json.dumps(spec.model_dump(), sort_keys=True, separators=(',', ':'))
        spec_digest = hashlib.sha256(spec_json_str.encode('utf-8')).hexdigest()
        assert spec_digest == res_json["spec_digest"]
        ...
        if evaluable:
            ...
            assert s_u.numerator == payload_dict["s_u"]["numerator"]
            assert s_u.denominator == payload_dict["s_u"]["denominator"]
            assert q_u.numerator == payload_dict["q_u"]["numerator"]
            assert q_u.denominator == payload_dict["q_u"]["denominator"]
            assert delta_u.numerator == payload_dict["delta_u"]["numerator"]
            assert delta_u.denominator == payload_dict["delta_u"]["denominator"]
            q_vals.append(q_u)
            
    # Recompute suite math
    assert agg["K"] == 11
    assert agg["H_0_75"] == 4
    
    q_suite = exact_median(q_vals)
    assert q_suite.numerator == agg["q_suite"]["numerator"]
    assert q_suite.denominator == agg["q_suite"]["denominator"]
    
    assert agg["classification"] == "WEAK_SUPPORT"
